"""
Full evaluation for Track 1 classical / geometry-based controllers.

Companion to `tools/eval/eval_track3.py` and `tools/eval/eval_track2.py`
(metaheuristics). Runs Random plus the five Track 1 controllers —
Boustrophedon, Spiral, VoronoiPartition, GridDecomposition, STC — at
their tuned-default configs over the same (map × n_drones × seed) grid,
dumps a CSV, and saves comparison plots.

Each controller's defaults come from its BO-tuned values in `tools/tuning/bo_search.py`.

Outputs (new runs — archived results stay without the _new suffix):
    outputs/classical/eval_track1_results_new.csv
    outputs/classical/eval/png/eval_track1_{coverage_vs_drones,...}_new.png
    outputs/classical/eval/svg/eval_track1_{...}_new.svg

Usage:
    python tools/eval/eval_track1.py
"""
...
from constants import MAPS_DIR, CLASSICAL_DIR
from controllers import (
    BoustrophedonController, GridDecompositionController,
    SpiralController, STCController, VoronoiPartitionController,
)
...
POLICY_COLORS = {
    "Random":            "#888888",
    "Boustrophedon":     "#1f77b4",
    "Spiral":            "#9467bd",
    "VoronoiPartition":  "#2ca02c",
    "GridDecomposition": "#ff7f0e",
    "STC":               "#d62728",
}
POLICY_ORDER = [
    "Random", "Boustrophedon", "Spiral",
    "VoronoiPartition", "GridDecomposition", "STC",
]
...
def make_policy(name: str) -> Optional[Callable]:
    """Fresh per (n_drones, seed) cell — Track 1 controllers carry plan state."""
    if name == "Boustrophedon":
        return BoustrophedonController(hover_drone_idx=None)
    if name == "Spiral":
        return SpiralController(hover_drone_idx=None)
    if name == "VoronoiPartition":
        return VoronoiPartitionController(hover_drone_idx=None)
    if name == "GridDecomposition":
        return GridDecompositionController(hover_drone_idx=None)
    if name == "STC":
        return STCController(hover_drone_idx=None)
    return None


def run_one(policy_name: str, *, grid: np.ndarray, map_label: str,
            n_drones: int, seed: int) -> RunResult:
    ...
    if policy_name == "Random":
        ...
        controller = make_policy(policy_name)
        ...
        for policy in POLICY_ORDER:
            ...
            for n in drones_list:
                vals = [
                    ...
                    if r.map_label == mlabel and r.policy == policy
                    and r.n_drones == n
                ]
                if not vals:
                    continue
            ...
            if xs:
                ax.errorbar(
                    xs, means, yerr=stds, label=policy,
...
def plot_coverage_vs_drones(results, drones_list, out_path):
    _facet_by_map(
        ...
        title="Track 1 — Final coverage vs swarm size  ↑ better",
...
def plot_time_to_80_vs_drones(results, drones_list, out_path):
    ...
    _facet_by_map(
        ...
        ylabel="Sim seconds",
        title="Track 1 — Time to 80% coverage  ↓ better\n"
...
def plot_efficiency_vs_drones(results, drones_list, out_path):
    ...
    _facet_by_map(
        ...
        title="Track 1 — Re-coverage waste vs swarm size  ↓ better",
        out_path=out_path, ylim_bottom=0,
    )
...
def plot_curves_grid(results, drones_list, out_path):
    ...
    fig, axes = plt.subplots(
        n_rows, n_cols, figsize=(3.4 * n_cols, 2.8 * n_rows),
    ...
    fig.suptitle("Track 1 — Coverage curves (rows=maps, cols=swarm size)",
...
def main():
    ...
    print(f"\n=== Track 1 sweep — {total} runs total ===")
    ...
    for mlabel, mgrid in maps:
        for n in args.drones_list:
            for policy in policies:
                for seed in seeds:
                    t_run0 = time.time()
                    r = run_one(policy, grid=mgrid, map_label=mlabel,
                    ...
                    print(f"  {mlabel:<12s} n={n:>2d}  {policy:<18s}  "
    ...
    csv_path = CLASSICAL_DIR / "eval_track1_results_new.csv"
    ...
    eval_png = CLASSICAL_DIR / "eval" / "png"
    eval_svg = CLASSICAL_DIR / "eval" / "svg"
    eval_png.mkdir(parents=True, exist_ok=True)
    eval_svg.mkdir(parents=True, exist_ok=True)
    plot_specs = [
        ("eval_track1_coverage_vs_drones_new",  plot_coverage_vs_drones),
        ("eval_track1_time_to_80_vs_drones_new", plot_time_to_80_vs_drones),
        ("eval_track1_efficiency_vs_drones_new", plot_efficiency_vs_drones),
        ("eval_track1_curves_grid_new",          plot_curves_grid),
    ]
    ...
    for mlabel, _ in maps:
        print(f"\n  {mlabel}:")
        header = "    n  " + "  ".join(f"{p:>17s}" for p in POLICY_ORDER)
        print(header)
        for n in args.drones_list:
            row = f"    {n:>1d}  "
            for p in POLICY_ORDER:
                vals = [r.final_coverage for r in results
                        if r.map_label == mlabel and r.policy == p
                        and r.n_drones == n]
                if vals:
                    row += f"  {np.mean(vals) * 100:>16.1f}%"
                else:
                    row += f"  {'—':>17s}"
            print(row)
