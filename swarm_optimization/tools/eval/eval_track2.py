"""
Full evaluation for Track 2 metaheuristic controllers.

Companion to `tools/eval/eval_track3.py`. Runs Random plus the five
...
Re-run with non-default configs by importing the controllers in your own script if needed.

Outputs (new runs — archived results stay without the _new suffix):
    outputs/metaheuristic/eval_track2_results_new.csv
    outputs/metaheuristic/eval/png/eval_track2_{coverage_vs_drones,...}_new.png
    outputs/metaheuristic/eval/svg/eval_track2_{...}_new.svg

Usage:
    python tools/eval/eval_track2.py \\
...
from constants import MAPS_DIR, METAHEURISTIC_DIR
...
# ---------------------------------------------------------------------------
# plotting (mirror tools/eval/eval_track3.py)
# ---------------------------------------------------------------------------
...
def main():
    ...
    METAHEURISTIC_DIR.mkdir(parents=True, exist_ok=True)
    csv_path = METAHEURISTIC_DIR / "eval_track2_results_new.csv"
    ...
    eval_png = METAHEURISTIC_DIR / "eval" / "png"
    eval_svg = METAHEURISTIC_DIR / "eval" / "svg"
    eval_png.mkdir(parents=True, exist_ok=True)
    eval_svg.mkdir(parents=True, exist_ok=True)
    plot_specs = [
        ("eval_track2_coverage_vs_drones_new",  plot_coverage_vs_drones),
        ("eval_track2_time_to_80_vs_drones_new", plot_time_to_80_vs_drones),
        ("eval_track2_efficiency_vs_drones_new", plot_efficiency_vs_drones),
        ("eval_track2_curves_grid_new",          plot_curves_grid),
    ]
