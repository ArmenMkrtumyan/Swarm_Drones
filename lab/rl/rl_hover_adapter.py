...
Usage (with the existing SITL + Isaac bridge already running):
    ...
    lab/rl/.rl_venv/Scripts/python.exe -m lab.rl.rl_hover_adapter \\
        --algo PPO \\
...
``lab.control.benchmark_hover`` post-processes for the apples-to-apples
metrics. The sidecar is purely for inspecting what the policy did.

Observation / action specs match :mod:`lab.rl.envs.hover_pid_tuner_v0`,
...
from lab.rl.envs.hover_pid_tuner_v0 import DEFAULT_GAINS, GainSpec


log = logging.getLogger("lab.rl.adapter")


def load_gain_spec(path: str | Path) -> list[GainSpec]:
    """Load the `gains:` block from a training-config yaml (e.g. compare_v1.yaml).

    The yaml schema matches what `lab.rl.config_loader.load_config` parses
...
def main() -> int:
    ...
    ap.add_argument("--gain-spec", default=None,
                    help="Path to a training-config yaml (e.g. lab/rl/cfg/compare_v1.yaml) "
