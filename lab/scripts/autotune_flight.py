...
Key parameters (set in sitl/params.parm):
...
def dump_tuned_params(master):
    ...
    with open(out_path, "w") as f:
        ...
        f.write(f"# Source params: sitl/params.parm baseline P/I=0.03, D=0.006\n")
        f.write(f"# Paste relevant values back into sitl/params.parm.\n")
        f.write(f"#\n")
        for name in TUNED_PARAMS:
            v = values.get(name)
            if v is not None:
...
def main():
    ...
    if outcome == "success":
        ...
        print("  Paste relevant values into sitl/params.parm,")
        print("  then re-run debug_arm_takeoff.py to verify.")
    else:
        print(f"  AUTOTUNE finished with outcome: {outcome}.")
        print("  Gains are NOT saved; sitl/params.parm baseline is unchanged.")
