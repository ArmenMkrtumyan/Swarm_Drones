...
a benchmark package:
  ...
  reports/benchmark_hover_report/
    ...
    comparison.png          -- cross-profile bar chart (only if 2+ profiles)
                              ...
                              use `lab.control.compare_batches` instead.

Without this benchmark locked, "RL beat ArduCopter" is unmeasurable. Sister
tool for Stage-2 missions: lab.missions.benchmark_mission.

Usage:
    python -m lab.control.benchmark_hover logs/flight_logs/
    python -m lab.control.benchmark_hover logs/flight_logs/ --out reports/benchmark_hover_report
    python -m lab.control.benchmark_hover logs/flight_logs/ --no-plots       # CSV/JSON only
    python -m lab.control.benchmark_hover logs/flight_logs/ --require-event  # official:
                                                                              # only logs from
                                                                              # the new bridge
                                                                              # with a chosen
                                                                              # profile
...
from lab.common.logging import FlightLog, load
from lab.control.metrics import (
    PROFILE_GATES,
    HoverMetrics,
    compute_metrics,
)
...
def describe_profile_parts(
    ...
    from lab.control.disturbance import make as make_profile
...
# paste, run `python -c "from lab.control.metrics import PROFILE_GATES;
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.no_plots and csv_rows:
        ...
        # comparison is generated separately by lab.control.compare_batches.
