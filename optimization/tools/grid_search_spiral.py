"""
Grid search over `SpiralController` hyperparameters across all maps × drone sizes.

Sweeps `pitch`, `attract_gain`, `drone_repel_gain`.

Outputs (to `outputs/csv_txt/` and `outputs/png/`):
    grid_search_spiral.csv
    grid_search_spiral_top.txt
    grid_search_spiral_heatmap.png

Usage:
    python tools/grid_search_spiral.py
"""
...
from controllers.spiral import SpiralConfig, SpiralController
...
def run_one(grid, *, map_label, n_drones, seed,
            pitch, attract_gain, drone_repel_gain):
    env = CoverageEnv(grid=grid, n_drones=n_drones,
                      sim=SimConfig(), drone=DroneConfig())
    env.reset(seed=seed)
    cfg = SpiralConfig(
        pitch=pitch, attract_gain=attract_gain,
        drone_repel_gain=drone_repel_gain,
    )
    ctl = SpiralController(cfg=cfg, hover_drone_idx=None)
...
def plot_heatmap(results, grid_axes, out_dir: Path) -> Path:
    """One combined PNG: 1 panel per pitch, attract × drone_repel per panel."""
    pitches = grid_axes["pitch"]
    attracts = grid_axes["attract_gain"]
    repels = grid_axes["drone_repel_gain"]

    agg: dict = {}
    for r in results:
        agg.setdefault((r.pitch, r.attract_gain, r.drone_repel_gain),
                       []).append(r.score)
    ...
    fig, axes = plt.subplots(1, len(pitches),
                             figsize=(4 * len(pitches) + 0.5, 3.6),
                             sharey=True)
    if len(pitches) == 1:
        axes = [axes]
    for ax, p in zip(axes, pitches):
        Z = np.array([
            [mean_score.get((p, a, r), float("nan")) for r in repels]
            for a in attracts
        ])
        ...
        ax.set_title(f"pitch = {p}", fontsize=10)
        for i, a in enumerate(attracts):
            for j, r in enumerate(repels):
                v = mean_score.get((p, a, r), None)
                if v is not None:
                    is_best = (p, a, r) == best_cfg
                    color = "white" if v < (vmin + vmax) / 2 else "black"
                    ax.text(j, i, f"{v:.2f}", ha="center", va="center",
                            color=color, fontsize=8,
                            fontweight="bold" if is_best else "normal")
    fig.suptitle(
        f"Spiral — grid search composite scores  (best: {best_score:+.3f})",
        fontsize=11, fontweight="bold",
    )
    fig.colorbar(im, ax=axes, fraction=0.025, pad=0.04, label="composite score")
    out = out_dir / "grid_search_spiral_heatmap.png"
...
def parse_args():
    ...
    p.add_argument("--pitch-grid",   type=float, nargs="+",
                   default=DEFAULT_GRID["pitch"])
...
def main():
    ...
    for p, ag, dg in configs:
        ...
        print(f"  [{runs_done}/{n_runs}] p={p} a={ag} dg={dg}  "
    ...
    csv_path = DATA_DIR / "grid_search_spiral.csv"
    fieldnames = [
        "pitch", "attract_gain", "drone_repel_gain",
    ...
    lines = []
    lines.append(f"Spiral grid search — top {args.top_k} configs\n\n")
    lines.append(f"{'rank':>4s}  {'pitch':>5s}  {'attract':>7s}  {'repel':>5s}  "
                 f"{'score':>7s}  {'cov':>6s}  "
    ...
    for rank, (key, sc, cov, ov, wa, en) in enumerate(summary[: args.top_k], 1):
        p, ag, dg = key
        lines.append(
            f"{rank:>4d}  {p:>5.2f}  {ag:>7.2f}  {dg:>5.2f}  "
            f"{sc:>+7.3f}  {cov:>5.1f}%  {ov:>7.0f}m²  {wa:>7.0f}  "
            f"{en:>6.1f}kJ\n"
        )
    top_path = DATA_DIR / "grid_search_spiral_top.txt"
