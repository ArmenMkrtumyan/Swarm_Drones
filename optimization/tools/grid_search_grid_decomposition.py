"""
Grid search over `GridDecompositionController` hyperparameters across all maps × drone sizes.

Sweeps `block_size`, `attract_gain`, `drone_repel_gain`.

Outputs (to `outputs/csv_txt/` and `outputs/png/`):
    grid_search_grid_decomposition.csv
    grid_search_grid_decomposition_top.txt
    grid_search_grid_decomposition_heatmap.png
"""
...
from controllers.grid_decomposition import (
    GridDecompositionConfig, GridDecompositionController,
)
...
def run_one(grid, *, map_label, n_drones, seed,
    ...
    return GRunResult(
        block_size=block_size, attract_gain=attract_gain,
        drone_repel_gain=drone_repel_gain,
...
def plot_heatmap(results, grid_axes, out_dir: Path) -> Path:
    """One PNG: 1 panel per block_size, attract × drone_repel per panel."""
    blocks = grid_axes["block_size"]
    attracts = grid_axes["attract_gain"]
    repels = grid_axes["drone_repel_gain"]

    agg: dict = {}
    for r in results:
        agg.setdefault((r.block_size, r.attract_gain, r.drone_repel_gain),
                       []).append(r.score)
    ...
    fig, axes = plt.subplots(1, len(blocks),
                             figsize=(4 * len(blocks) + 0.5, 3.6),
                             sharey=True)
    if len(blocks) == 1:
        axes = [axes]
    for ax, bs in zip(axes, blocks):
        Z = np.array([
            [mean_score.get((bs, a, r), float("nan")) for r in repels]
            for a in attracts
        ])
        ...
        ax.set_title(f"block_size = {bs}", fontsize=10)
        for i, a in enumerate(attracts):
            for j, r in enumerate(repels):
                v = mean_score.get((bs, a, r), None)
                if v is not None:
                    is_best = (bs, a, r) == best_cfg
                    color = "white" if v < (vmin + vmax) / 2 else "black"
                    ax.text(j, i, f"{v:.2f}", ha="center", va="center",
                            color=color, fontsize=8,
                            fontweight="bold" if is_best else "normal")
    fig.suptitle(
        f"GridDecomposition — grid search composite scores  (best: {best_score:+.3f})",
        fontsize=11, fontweight="bold",
    )
    fig.colorbar(im, ax=axes, fraction=0.025, pad=0.04, label="composite score")
    out = out_dir / "grid_search_grid_decomposition_heatmap.png"
...
def parse_args():
    ...
    p.add_argument("--block-grid", type=int, nargs="+",
                   default=DEFAULT_GRID["block_size"])
...
def main():
    ...
    for bs, ag, dg in configs:
        ...
        print(f"  [{runs_done}/{n_runs}] bs={bs} a={ag} dg={dg}  "
    ...
    csv_path = DATA_DIR / "grid_search_grid_decomposition.csv"
    fieldnames = [
        "block_size", "attract_gain", "drone_repel_gain",
    ...
    lines = []
    lines.append(f"GridDecomposition grid search — top {args.top_k} configs\n\n")
    lines.append(f"{'rank':>4s}  {'block':>5s}  {'attract':>7s}  {'repel':>5s}  "
                 f"{'score':>7s}  {'cov':>6s}  "
    ...
    for rank, (key, sc, cov, ov, wa, en) in enumerate(summary[: args.top_k], 1):
        bs, ag, dg = key
        lines.append(
            f"{rank:>4d}  {bs:>5d}  {ag:>7.2f}  {dg:>5.2f}  "
            f"{sc:>+7.3f}  {cov:>5.1f}%  {ov:>7.0f}m²  {wa:>7.0f}  "
            f"{en:>6.1f}kJ\n"
        )
    top_path = DATA_DIR / "grid_search_grid_decomposition_top.txt"
