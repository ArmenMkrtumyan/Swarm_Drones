"""
Grid search over `VoronoiPartitionController` hyperparameters across all maps × drone sizes.

Sweeps `attract_gain`, `drone_repel_gain`, `drone_repel_range`.

Outputs (to `outputs/csv_txt/` and `outputs/png/`):
    grid_search_voronoi_partition.csv
    grid_search_voronoi_partition_top.txt
    grid_search_voronoi_partition_heatmap.png

Usage:
    python tools/grid_search_voronoi_partition.py
"""
...
from controllers.voronoi_partition import (
    VoronoiPartitionConfig, VoronoiPartitionController,
)
...
def run_one(grid, *, map_label, n_drones, seed,
            attract_gain, drone_repel_gain, drone_repel_range):
    env = CoverageEnv(grid=grid, n_drones=n_drones,
                      sim=SimConfig(), drone=DroneConfig())
    env.reset(seed=seed)
    cfg = VoronoiPartitionConfig(
        attract_gain=attract_gain,
        drone_repel_gain=drone_repel_gain,
        drone_repel_range=drone_repel_range,
    )
    ctl = VoronoiPartitionController(cfg=cfg, hover_drone_idx=None)
    ...
    return VRunResult(
        attract_gain=attract_gain, drone_repel_gain=drone_repel_gain,
        drone_repel_range=drone_repel_range,
...
def plot_heatmap(results, grid_axes, out_dir: Path) -> Path:
    """One PNG: 1 panel per drone_repel_range, attract × drone_repel_gain per panel."""
    ...
    agg: dict = {}
    for r in results:
        agg.setdefault((r.drone_repel_range, r.attract_gain, r.drone_repel_gain),
                       []).append(r.score)
    ...
    fig, axes = plt.subplots(1, len(ranges),
                             figsize=(4 * len(ranges) + 0.5, 3.6),
                             sharey=True)
    if len(ranges) == 1:
        axes = [axes]
    for ax, rr in zip(axes, ranges):
        Z = np.array([
            [mean_score.get((rr, a, dr), float("nan")) for dr in repels]
            for a in attracts
        ])
        im = ax.imshow(Z, origin="lower", aspect="auto",
                       cmap="viridis", vmin=vmin, vmax=vmax)
        ax.set_xticks(range(len(repels)))
        ax.set_xticklabels([str(v) for v in repels], fontsize=8)
        ax.set_yticks(range(len(attracts)))
        ax.set_yticklabels([str(v) for v in attracts], fontsize=8)
        ax.set_xlabel("drone_repel_gain", fontsize=9)
        if ax is axes[0]:
            ax.set_ylabel("attract_gain", fontsize=9)
        ax.set_title(f"drone_repel_range = {rr}", fontsize=10)
        for i, a in enumerate(attracts):
            for j, dr in enumerate(repels):
                v = mean_score.get((rr, a, dr), None)
                if v is not None:
                    is_best = (rr, a, dr) == best_cfg
                    color = "white" if v < (vmin + vmax) / 2 else "black"
                    ax.text(j, i, f"{v:.2f}", ha="center", va="center",
                            color=color, fontsize=8,
                            fontweight="bold" if is_best else "normal")
    fig.suptitle(
        f"VoronoiPartition — grid search composite scores  (best: {best_score:+.3f})",
        fontsize=11, fontweight="bold",
    )
    fig.colorbar(im, ax=axes, fraction=0.025, pad=0.04, label="composite score")
    out = out_dir / "grid_search_voronoi_partition_heatmap.png"
...
def parse_args():
    ...
    p.add_argument("--repel-gain-grid", type=float, nargs="+",
                   default=DEFAULT_GRID["drone_repel_gain"])
    p.add_argument("--repel-range-grid", type=float, nargs="+",
                   default=DEFAULT_GRID["drone_repel_range"])
...
def main():
    ...
    csv_path = DATA_DIR / "grid_search_voronoi_partition.csv"
    fieldnames = [
        "attract_gain", "drone_repel_gain", "drone_repel_range",
    ...
    agg: dict = {}
    for r in results:
        agg.setdefault((r.attract_gain, r.drone_repel_gain, r.drone_repel_range),
                       []).append(r)
    summary = sorted(
        [
            (key,
    ...
    lines = []
    lines.append(f"VoronoiPartition grid search — top {args.top_k} configs\n\n")
    lines.append(f"{'rank':>4s}  {'attract':>7s}  {'repel_g':>7s}  {'repel_r':>7s}  "
                 f"{'score':>7s}  {'cov':>6s}  "
    ...
    for rank, (key, sc, cov, ov, wa, en) in enumerate(summary[: args.top_k], 1):
        ag, dg, dr = key
        lines.append(
            f"{rank:>4d}  {ag:>7.2f}  {dg:>7.2f}  {dr:>7.2f}  "
            f"{sc:>+7.3f}  {cov:>5.1f}%  {ov:>7.0f}m²  {wa:>7.0f}  "
            f"{en:>6.1f}kJ\n"
        )
    top_path = DATA_DIR / "grid_search_voronoi_partition_top.txt"
