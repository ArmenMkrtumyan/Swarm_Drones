"""
Grid search over `STCController` hyperparameters across all maps × drone sizes.

Sweeps `attract_gain`, `drone_repel_gain`, `drone_repel_range`.

Outputs (to `outputs/csv_txt/` and `outputs/png/`):
    grid_search_stc.csv
    grid_search_stc_top.txt
    grid_search_stc_heatmap.png
"""
...
from controllers.stc import STCConfig, STCController
...
def run_one(grid, *, map_label, n_drones, seed,
    ...
    ctl = STCController(cfg=cfg, hover_drone_idx=None)
...
def plot_heatmap(results, grid_axes, out_dir: Path) -> Path:
    ...
    fig.suptitle(
        f"STC — grid search composite scores  (best: {best_score:+.3f})",
        fontsize=11, fontweight="bold",
    )
    fig.colorbar(im, ax=axes, fraction=0.025, pad=0.04, label="composite score")
    out = out_dir / "grid_search_stc_heatmap.png"
...
def main():
    ...
    csv_path = DATA_DIR / "grid_search_stc.csv"
    fieldnames = [
        "attract_gain", "drone_repel_gain", "drone_repel_range",
    ...
    lines = []
    lines.append(f"STC grid search — top {args.top_k} configs\n\n")
    ...
    top_path = DATA_DIR / "grid_search_stc_top.txt"
