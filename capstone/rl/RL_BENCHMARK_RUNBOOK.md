...
Tools/autotest/sim_vehicle.py -v ArduCopter -f X -N -w \
  --add-param-file=...sitl_params_test.parm \
...
capstone\rl\.rl_venv\Scripts\python.exe -m capstone.control.benchmark_hover `
    reports\benchmark_hover_report\rl_calm_run `
...
```powershell
capstone\rl\.rl_venv\Scripts\python.exe -m capstone.control.compare_batches `
...
- `Swarm_Drones/capstone/rl/rl_hover_adapter.py` — the adapter (~200 LOC).
...
- `Swarm_Drones/capstone/control/benchmark_hover.py` — existing post-processor.
- `Swarm_Drones/capstone/control/compare_batches.py` — existing cross-batch plot.
